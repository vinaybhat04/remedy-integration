...
import org.opennms.plugin.ticketer.jira.integration.cache.Cache;
import org.opennms.plugin.ticketer.jira.integration.cache.TimeoutRefreshPolicy;
 ...
 * @author Seth
 */
public class JiraTicketerPlugin implements TicketingPlugin {
    ...
    */
    ...
    private void populateFields(Ticket ticket, IssueInputBuilder builder) {
        ...
        if (ticket.getAttributes() == null) {
            return;
        } else if (ticket.getAttributes().size() == 0) {
            return;
        }
